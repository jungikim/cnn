...
FastLSTMBuilder::FastLSTMBuilder(unsigned layers,
  ...
  for (unsigned i = 0; i < layers; ++i) {
    ...
    Parameters* p_c2i = model->add_parameters(vector_of<unsigned int>(hidden_dim)(1));
    ...
    Parameters* p_c2o = model->add_parameters(vector_of<unsigned int>(hidden_dim)(1));
...
//      i_ait = vars[BI] + vars[X2I] * in + vars[H2I]*i_h_tm1 + cwise_multiply(vars[C2I], i_c_tm1);
      i_ait = affine_transform(vector_of<Expression>(vars[BI])(vars[X2I])(in)(vars[H2I])(i_h_tm1)) +
...
//      i_aot = vars[BO] + vars[X2O] * in + vars[H2O] * i_h_tm1 + cwise_multiply(vars[C2O], ct[i]);
      i_aot = affine_transform(vector_of<Expression>(vars[BO])(vars[X2O])(in)(vars[H2O])(i_h_tm1)) +
